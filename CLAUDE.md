...
Currently working: **DH — Data Health epic** (read-only data-quality monitoring), owner-approved 2026-06-24 (Rule 8 Tier-2; spec in betvector_buildplan.md "## DH"). Owner picked ALL THREE surfaces. 4 issues: **01 check engine + config (DONE)** → **02 CLI `make health` (DONE)** → **03 🩺 Data Health dashboard page (NEXT)** → 04 morning-pipeline FAIL-alert + integration test (closes epic). Motivated by standings not filling (stale `scheduled` stubs); the engine is the live tripwire. Read-only (every check is a SELECT); config-driven; $0; ZERO change to model/value/predictions/bet logic. LIVE SMOKE CAUGHT THE REAL ISSUE: 2 league + 4 WC stale stubs right now. PRIOR epics: HC COMPLETE (6/6, v1.7); WC-11A (4/4, v1.6); DF (10/10, v1.5). Backlog (owner-directed): GATED WC-11 player-props overlay; PC-27 soak; owner task task_5a7fe3ba (Settings/onboarding flat-staking + drawdown COPY + flat-vs-current CODE decision).
