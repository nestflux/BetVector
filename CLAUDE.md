...
Currently working: WC-10-07 — rotation/absence flag on the research card (Tab 1). Compute "N starters rested / key player out" by comparing the captured XI (wc_lineups) to the team's regulars; surface a ⚠️ flag. DECISION-SUPPORT ONLY — no λ/value-bet change.
Next up: WC-10 COMPLETE after 10-07 (10-08 DEFERRED). Owner: log in to verify Bayes-vs-Poisson panel + the new lineup flag; PC-27 soak. Props=WC-11. KEY: ad-hoc scripts must load_dotenv("/Users/kyng/Projects/BetVector/.env") or hit SQLite. Odds API remaining ~313 (lineups are free via ESPN).
