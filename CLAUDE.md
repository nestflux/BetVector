...
Currently working: WC-10-04 — implement run_prematch for real (in pipeline.py): exactly ONE Odds-API pull (1 region) for the target match → refresh odds (near-closing line) + re-derive value/edge. Then INSTALL the dispatcher plist (was deferred so it doesn't fire placeholders). Budget-disciplined.
Next up: WC-10-05 CLV integrity (near-closing line → capture_wc_closing_lines on finish); Phase 3 lineup flag (10-06/07 via api_football); 10-08 DEFERRED. Owner: log in to verify Bayes-vs-Poisson panel; PC-27 soak. Props=WC-11. KEY: ad-hoc scripts must load_dotenv("/Users/kyng/Projects/BetVector/.env") or they hit SQLite. Odds API remaining ~315.
