...
Currently working: WC-10-05 — CLV integrity (verify a prematch-captured near-closing line flows into capture_wc_closing_lines on finish → true-close CLV on the scorecard). Does NOT need the dispatcher installed.
HOLDING: dispatcher INSTALL (com.betvector.wc_dispatcher.plist) — it's a 15-min credit-spending job (~2 credits/match, ~200/mo). Awaiting explicit owner OK before launchctl load. Code is inert until installed.
