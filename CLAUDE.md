...
Last completed: WC-10-01 ✅ — Diagnose & harden WC odds refresh. ROOT CAUSE: the "0 odds on Neon" was a TOOLING ARTIFACT — ad-hoc /tmp scripts calling bare load_dotenv() don't find .env → db.py SILENTLY falls back to local SQLite (resolution: DATABASE_URL → st.secrets → SQLite). Production was always consistent (pipeline sources .env → Neon; dashboard → st.secrets → Neon; Neon had 6,627 odds throughout). Fixes: (a) scrape default 12→2 credits (config odds_scrape: h2h,totals/eu, eu incl. Pinnacle; was h2h,spreads,totals × 4 regions); (b) LOUD warning on db.py SQLite fallback; (c) per-run fetched/matched/skipped log; (d) repopulated Neon (odds 6,781 fresh + Bayesian preds 0→40 — earlier shadow runs had hit SQLite). 6 tests, 719/719. Odds API remaining: 319.
Currently working: WC-10-02 — Daily morning automation (retimed ~09:30 ET; settle overnight results + board + predictions + Bayesian + value bets + email; install morning plist, retire 22:00 evening plist). Needs owner OK for the launchd install (persistent config).
Next up (WC-10): 10-03/04/05 dispatcher + closing-line CLV; 10-06/07 lineup capture + rotation flag; 10-08 DEFERRED. Owner: log in to verify Bayes-vs-Poisson panel (now has data on Neon); PC-27 soak. Props=WC-11 (deferred). KEY: ad-hoc scripts must load_dotenv("/Users/kyng/Projects/BetVector/.env") explicitly or they hit SQLite.
