...
Currently working: **NONE — HC (Help Center) epic COMPLETE ✅ (6/6, masterplan v1.7).** All six done: 01 glossary → 02 screen tour → 03 FAQ + on-page links → 04 Betting 101 → 05 interactive tools → 06 downloadable doc + glossary migration. Owner picked: in-app page + downloadable doc, FULL depth, per-page "How to read this page" links. Read-only/content-driven; ZERO change to model/value/predictions/bet logic; $0 cost. PRIOR epics: WC-11A COMPLETE (4/4, v1.6); DF COMPLETE (10/10, v1.5). Backlog (owner-directed, none auto-advancing): GATED WC-11 player-props overlay (≈ whole monthly Odds API quota); PC-27 7-day soak watch; owner task task_5a7fe3ba (reconcile the app's Settings/onboarding flat-staking + drawdown COPY, and the flat-vs-current CODE decision). Next sequential issue: none queued — await owner direction.
