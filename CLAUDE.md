...
Currently working: **DH — Data Health epic** (read-only data-quality monitoring), owner-approved 2026-06-24 (Rule 8 Tier-2; spec in betvector_buildplan.md "## DH"). Owner picked ALL THREE surfaces. 4 issues: **01 check engine + config (DONE)** → **02 CLI `make health` (DONE)** → **03 🩺 Data Health dashboard page (DONE)** → **04 morning-pipeline FAIL-alert + integration test (NEXT — closes epic)**. Motivated by standings not filling (stale `scheduled` stubs); the engine is the live tripwire. Read-only (every check is a SELECT); config-driven; $0; ZERO change to model/value/predictions/bet logic. LIVE SMOKE CAUGHT THE REAL ISSUE: 2 league + 4 WC stale stubs right now. PRIOR epics: HC COMPLETE (6/6, v1.7); WC-11A (4/4, v1.6); DF (10/10, v1.5). Backlog (owner-directed): GATED WC-11 player-props overlay; PC-27 soak; owner task task_5a7fe3ba (Settings/onboarding flat-staking + drawdown COPY + flat-vs-current CODE decision).
Last done: HC-06 ✅ — Downloadable manual + glossary migration (CLOSES the HC epic). (a) Two pure builders in help_content.py: build_manual_markdown() (START_HERE + TOUR + CONCEPTS + FAQ + GLOSSARY_GROUPS → one Markdown string) + build_manual_html() (self-contained print→PDF HTML, light theme, escaped) — NO new dependency; surfaced as two st.download_buttons on the Help Start-here tab (betvector_manual.md/.html). (b) Single-sourced the 5 page glossaries: new GLOSSARY_BY_TERM (69-term master lookup) + PAGE_GLOSSARIES (per-page sections/labels; shared concepts pull defs via _T["master term"], page-only stat/UI labels keep a local def) + pure escaped glossary_sections_html(page_key) renderer. picks/performance/bankroll/match_detail/wc_deep_dive each swapped their inline-HTML glossary for ONE glossary_sections_html("<page>") call (kept own CSS+expander) — net −634 lines, behaviour-preserving (same terms/sections/labels), and the HC-04 fixes (drawdown 25% not 30%, flat=CURRENT bankroll) PROPAGATE to the page glossaries from the master. Importing help_content KeyErrors on any mistyped master term (built-in single-source guard). READ-ONLY/$0: only help_content.py + 6 views + 3 tests; value_finder.py ×2 + predictor.py byte-for-byte unchanged. 11 new tests (lookup==master, behaviour preservation, propagated fixes, escaping, both builders+completeness, source-level wiring ×5), 1017/1017. Gate 1 PASS / Gate 2 CLEAN (every migrated def re-verified vs code/config; 1 precision fix: "Anytime scorer %" → "adjusted expected goals" to match the scorer-board λ) / Gate 3 APPROVED (escaping + single-source + no import cycle). Real PNGs on owner Desktop (print-friendly manual + migrated Bankroll glossary showing 25%/current). Rule-8 Tier-1: masterplan §13.16 HC paragraph + version 1.6→1.7. LESSON: when centralising duplicated copy, diff the OLD page text against the new shared def term-by-term — a consolidation can silently drop a qualifier ("adjusted") that the code actually relies on.
