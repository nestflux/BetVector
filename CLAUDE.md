...
Currently working: WC-10-01 — Diagnose & harden the WC odds refresh (board emptied to 0 on Neon while predictions remained). Part of new epic WC-10 Live Operations & Automation (owner-confirmed): P1 daily automation, P2 dynamic pre-kickoff CLV dispatcher, P3 lineup rotation flag. Spec in worldcup_buildplan.md "## WC-10". No model/staking change.
Next up (WC-10): 10-02 retimed morning automation; 10-03/04/05 dispatcher + closing-line CLV; 10-06/07 lineup capture + rotation flag; 10-08 DEFERRED (λ-adjust). Owner tasks: log in to verify Bayes-vs-Poisson panel; PC-27 soak. Props build renumbered WC-11 (deferred). NOTE: WC odds API name maps may be the empty-board cause — check WC-10-01.
