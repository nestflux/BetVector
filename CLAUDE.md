...
Currently working: DF-08 next — WC Deep Dive: Scaffold + Heatmap + Model-vs-Books (Phase B start). New WC deep-dive view mirroring src/delivery/views/match_detail.py against the WC tables (WCMatch/WCPrediction/WCOdds/Bayesian): scoreline 7×7 heatmap (rebuild via predictor.derive_markets_from_lambdas / the scoreline matrix from λh,λa) + model-vs-EVERY-pulled-book visual comparison for all markets (port _build_bookie_probs / _render_prob_cell from match_detail.py). Entry buttons on each fixture row + the research card (session-state match id + nav switch). Spec worldcup_buildplan.md §DF-08 ~line 1657. ACs: click fixture/research-card opens that match's deep dive; heatmap renders; model vs every book per market; empty/missing-data states. CAUTION: WC = shadow; reuse research.build_research_card/summarize_card (already returns all markets + blocks) + derive_markets_from_lambdas (heatmap). Phase A (DF-01..07) all DONE + DF-06 verified LIVE on betvector.streamlit.app.
Last done: DF-07 ✅ — Biggest Disagreements Redesign (completes Phase A). Reworked the research card's "Biggest disagreements" queue from a flat dataframe into ranked verdict sentences. New pure research.build_disagreements(limit,cfg) collapses each market to the side the MODEL favours (one directional call, not a mirror pair), keeps it only if edge ≥ threshold (a real disagreement), tags via DF-06 _edge_trust against the value-finder bounds: value→✓ conviction (backable shadow lean + best price), capped→⚠ likely model error (past ceiling). Sort (conviction-before-capped, edge desc) → trustworthy calls lead even when a capped gap is bigger (+28% capped ranks below +13% conviction). _disagreement_sentence (pure) writes the sentence; view _disagreement_row_html adds the ✓/⚠ tag + signed-edge rank marker (escaped). Empty/in-line/no-pred → [] + neutral caption. top_disagreements kept as the tested primitive. value_finder.py/predictor.py byte-for-byte unchanged (shadow). 831/831 (+9), Gate 2 CLEAN / Gate 3 APPROVED. PNG on owner Desktop. Pushed 1829469.
