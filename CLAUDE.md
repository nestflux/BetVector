...
Currently working: NOTHING — WC-10 COMPLETE (7/7, 10-08 deferred). WHOLE WC MODULE (37 issues) DONE + LIVE. League system (242) done. The WC system runs itself: 09:30 ET morning job + 15-min dispatcher (prematch odds/CLV + ESPN lineups), all on Neon.
Next up: No queued issues. Owner: (1) log in to verify Bayes-vs-Poisson panel + lineup flag; (2) PC-27 7-day soak; (3) watch the CLV scorecard fill now that the dispatcher is live. DEFERRED: 10-08 λ-adjust + props = WC-11 (gated). KEY: ad-hoc scripts must load_dotenv("/Users/kyng/Projects/BetVector/.env") or hit SQLite. Odds API ~313 (lineups free via ESPN).
