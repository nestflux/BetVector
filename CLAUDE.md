...
Also done: WC-10-04 ✅ — focused per-event prematch pull. scraper.scrape_wc_match_odds(match_id): FREE /events lookup → ONE paid /events/{id}/odds pull (h2h,totals/eu = 2 credits) → upsert ONLY that match (no board churn — protects in-play matches' closing lines). pipeline.run_prematch now real (focused pull + value recompute on ok). Verified on real API (match 17, 315→313 credits, idempotent). 8 tests, 741/741. 3-gate all green.
Also done: WC-10-05 ✅ — CLV integrity verified. Test: real _load_odds_to_db upsert (morning 2.00 → prematch 1.50, one row) → capture_wc_closing_lines → close=1.50 (latest/near-closing, NOT 2.00 morning) + CLV>0. Real Neon: 4 settled picks have CLV captured but ALL read 0.0 (close==entry==morning line, finished before any prematch refresh). → meaningful CLV is BLOCKED until the dispatcher is live. 2 tests, 743/743. → WC-10 Phase 2 CODE COMPLETE (5/7).
Currently working: PAUSED — WC-10 Phase 2 code complete. ONE decision pending owner: INSTALL the dispatcher? It's the 15-min credit-spending job (com.betvector.wc_dispatcher.plist, ~2 credits/match, ~200/mo). It's what turns the CLV scorecard from all-0.0 into a real signal. Trivially reversible (launchctl unload). Code is inert until loaded. Asked twice; awaiting explicit OK.
Next up (on install OK): cp scripts/com.betvector.wc_dispatcher.plist ~/Library/LaunchAgents/ && launchctl load it; verify in launchctl list. Then Phase 3 lineup flag (10-06/07 via api_football); 10-08 DEFERRED. Owner: log in to verify Bayes-vs-Poisson panel; PC-27 soak. Props=WC-11. KEY: ad-hoc scripts must load_dotenv("/Users/kyng/Projects/BetVector/.env") or hit SQLite. Odds API remaining ~313.
