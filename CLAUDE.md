...
Currently working: DF-09 next — WC Deep Dive: Movement + Lineups (extends the DF-08 page src/delivery/views/wc_deep_dive.py). Add a line-movement chart (price since open, with entry + close marked — the CLV story) + both confirmed XIs/formations + the rotation flag (reuse lineups.lineup_signal). Spec worldcup_buildplan.md §DF-09. ACs: movement chart shows price history w/ entry+close markers; both lineups+formations when available (graceful "not announced"); rotation flag (decision-support framing); NO model/value change. REUSE: WCOdds.opening_odds (frozen first price) for movement; WCValueBet.closing_odds/clv for close; lineups.lineup_signal already powers the research-card flag in world_cup.py (_render_lineup_flag). Then DF-10 (qualification impact + per-match Bayesian-vs-Poisson read + glossary + nav/integration test). Phase A (DF-01..07) DONE; DF-08 DONE (Phase B scaffold).
Last done: DF-08 ✅ — WC Deep Dive: Scaffold + Heatmap + Model-vs-Books (Phase B start). New read-only page src/delivery/views/wc_deep_dive.py mirroring match_detail.py against the WC tables. 3 pure additions (value path byte-for-byte unchanged — shadow): predictor.scoreline_matrix_from_lambdas (rebuilds the 7×7 Dixon-Coles grid from stored λ — wc_predictions stores λ not the matrix); research.build_book_comparison + _collect_by_book (per market 1X2+O/U1.5/2.5/3.5+BTTS: model prob + de-vigged consensus + EVERY pulled book's own de-vigged line, edge-tagged via the DF-06 _trust_bounds/_edge_trust config bounds, not hardcoded); view _market_table_html (pure) renders model row / consensus row / one row per book, edge-tinted (green=value vs that book, amber=past ceiling/likely model error), softest-book-first, ★ on best price. Entry: WC hub fixtures strip (per-row expander button) + research card both set st.session_state["wc_deep_dive_match_id"] + st.switch_page; page resolves session-state (one-shot pop)→?wc_match_id, registered in dashboard.py nav, picker fallback. Empty/missing: no pred→no heatmap; no odds→no comparison; not found→st.error+back. 851/851 (+20), Gate 1 PASS 4/4 / Gate 2 CLEAN / Gate 3 APPROVED (3 dead-code nits fixed). Real heatmap+model-vs-books PNG on owner Desktop.
