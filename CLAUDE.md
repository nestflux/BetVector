...
Currently working: **WC-11A — Player Insight (display-only, shadow)** epic, owner-approved 2026-06-24, spec in worldcup_buildplan.md "## WC-11A". 4 issues: 01 rate engine + resolver (DONE) → 02 display-only adjusted-λ (DONE) → 03 scorer board + pen-taker (DONE) → **04 player-watch extras (NEXT)**. All display-only; value path (value_finder.py ×2) + predictor.py byte-for-byte unchanged. Also done this run (post-DF): WC banner "days to final" bug FIXED (68b8c8f — config-driven timeutil.days_to_final, counts to tournament.end_date not max fixture). Forza Football evaluated as a data source + REJECTED (no public API, bot-blocked, overlaps ESPN/Odds/TM; recorded in reference_data_sources memory). DF EPIC COMPLETE (10/10, masterplan v1.5).
Last done: WC-11A-03 ✅ — "who's likely to score" board + penalty-taker flag (display-only, zero Odds API credits). New pure research.build_scorer_board(match_id, rate_lookup) reuses the WC-11A-02 read (_match_legs, factored out of build_lineup_impact so both share ONE DB pass) + _team_impact wholesale (single source of truth for the λ math), takes each in-XI player's exp_goals (= goal-share × adjusted team λ, the 11A-02 slice) as player_λ, and turns it into an anytime chance P = 1 − exp(−player_λ) via _anytime_prob (Poisson P(≥1); guards λ≤0/None → None, never a guess). Each player carries is_pen_taker + the goals-per-90 source ('club'/'international') re-read via the SAME rate_lookup/full_name the impact layer used; ranked P desc; unrated + rotated-out excluded; a zero-rate keeper is silently dropped (rated zero, not "unrated"). Pen taker is FLAGGED NOT BUMPED — his spot-kicks are already in his goals-per-90 (a bump would double-count). NO odds pull anywhere → zero credits. Deep-dive Section 7 _render_scorer_board (after lineup impact, before group context): per-team ranked card (#, player + PK/intl chips, g/90, anytime % with a NEUTRAL grey proportional bar — an estimate, not an edge), not-announced/no-model/all-unrated states, shadow caption, PK/intl + unrated footnotes; glossary +"Anytime scorer %"/"Penalty taker". READ-ONLY: no add/commit, nothing written back; predictor.py + value_finder.py ×2 EMPTY diff. 14 tests (formula + ranking + flags + zero-rate drop + read-only + escaping + view AST), 956/956. Gate 1 PASS 6/6 / Gate 2 CLEAN / Gate 3 APPROVED (two cosmetic nits; the zero-rate-drop one pinned with a test, the other accepted). Real-rate PNG on owner Desktop: England Kane 52.5% [PK] top, defenders low (Shaw 1.5%); Portugal Ronaldo 41.4% [PK,intl].
