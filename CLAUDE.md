...
Prior: WC-10-01 ✅ — fixed the SQLite/Neon split-brain footgun (ad-hoc /tmp scripts hit SQLite via bare load_dotenv → db.py now WARNS loudly on SQLite fallback) + scrape budget 12→2 credits (config odds_scrape: h2h,totals/eu, eu incl. Pinnacle) + repopulated Neon (odds fresh + Bayesian 0→40). WC-09 (36/36) complete before that.
Currently working: PAUSED — WC-10 Phase 1 (daily automation) COMPLETE (2/7). System self-refreshes at 09:30 ET daily.
Next up: WC-10 Phase 2 — dynamic pre-kickoff CLV dispatcher (10-03 local-cache heartbeat → 10-04 prematch closing-line run → 10-05 CLV integrity); Phase 3 lineup flag (10-06/07); 10-08 DEFERRED. Owner: log in to verify Bayes-vs-Poisson panel (data on Neon now); PC-27 soak. Props=WC-11. KEY: ad-hoc scripts must load_dotenv("/Users/kyng/Projects/BetVector/.env") explicitly or they hit SQLite (Odds API remaining ~315).
