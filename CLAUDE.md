...
Currently working: **HC — Help Center epic** (in-app user manual), owner-approved 2026-06-24 (Rule 8 Tier-2, spec in betvector_buildplan.md "## HC"). 6 issues, 3 phases: 01 glossary (DONE) → 02 screen tour (DONE) → 03 FAQ + on-page links (DONE) → 04 Betting 101 (DONE) → **05 interactive tools (DONE)** → **06 downloadable doc + migrate the 5 page glossaries (NEXT — closes the epic)**. Owner picked: in-app page + downloadable doc, FULL depth, per-page "How to read this page" links. Read-only/content-driven; ZERO change to model/value/predictions/bet logic; $0 cost. PRIOR epics: WC-11A COMPLETE (4/4, masterplan v1.6); DF COMPLETE (10/10, v1.5). Other backlog (owner-directed): GATED WC-11 player-props market overlay (≈ whole monthly Odds API quota); PC-27 7-day soak watch.
Last done: HC-05 ✅ — Help Center interactive Tools (🧮 Tools tab, 6th). Pure math helpers in help_content.py (unit-tested EXACT): implied_pct_from_odds(100/odds), edge_pp(model−raw 1/odds, matches value_finder), verdict_for_edge(value/capped/none on config bounds, inclusive both ends), flat_stake(bankroll×stake%, matches bankroll.py), kelly_fraction_of_bankroll(f*=(p·odds−1)/(odds−1)) + kelly_stake(×configured kelly_fraction). View supplies live st.number_inputs + reads the SAME config bounds via _tool_bounds() (edge_threshold 0.03/max_actionable_edge 0.15/kelly_fraction 0.25 — lazy-loaded, documented fallback, NOT hardcoded), renders 3 pure escaped read-outs: "Is it value?" (implied%, edge pp coloured, VALUE/CAPPED/NO-EDGE pill), "How much to stake?" (flat + ¼-Kelly $), and a scoreline-matrix reader (labelled 7×7, home/draw/away tinted + legend deriving 1X2/O2.5/BTTS from cells). All inputs numeric → no XSS surface; read-only. 6 new tests, 1008/1008. Gate 1 PASS / Gate 2 CLEAN (every formula re-derived vs value_finder/bankroll/base_model + config) / Gate 3 APPROVED (crash-safe: tab unpack 6→6, division guards, no import cycle; 1 unreachable-NaN nit, no fix). Real PNG on owner Desktop. Only help_content.py + views/help.py + test changed.
