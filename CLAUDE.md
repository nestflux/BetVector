...
Currently working: DF-01 next — Market expansion. predictor.py ALREADY computes over_25/over_35/btts from the matrix; only over_25_prob+btts_prob are stored in WCPrediction. DF-01: add over_15, persist over_15_prob+over_35_prob (2 cols + migration on Neon+SQLite), split odds config (board pull h2h,totals,btts,alternate_totals vs lean per-event h2h,totals), surface all markets in research.py. ~+2 credits/day.
Last done: DF-02 ✅ — WC = login landing during the tournament window. config tournament.start/end_date (2026-06-11→07-19); timeutil.wc_window_active/tournament_window (lru_cached, Eastern today, inclusive, safe None fallback); dashboard.get_pages flips default Fixtures↔World Cup + floats WC to top when active; auto-reverts day after the final. 11 tests, 774/774. 3-gate green.
