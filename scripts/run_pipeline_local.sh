...
if [[ -f "$ENV_FILE" ]]; then
    ...
    # whole pipeline run under `set -e`. run_pipeline.py also calls load_dotenv(),
    # which parses such quirks correctly and is the real loader, so this shell
    # source is best-effort. stderr -> /dev/null so a bad line can never echo a
    # secret value into the logs.
    set +e
    set -a
