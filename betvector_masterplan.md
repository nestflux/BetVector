...
3. Step 2 — Staking: "How do you want to calculate stakes?" Three options with explanations:
   - Flat Stakes (recommended for beginners): "Bet a fixed 2% of your starting bankroll every time — the same stake regardless of swings. Simple and safe."
...
- Three staking methods: flat stakes (a level 2% of the **starting** bankroll — the same fixed amount every bet), percentage of the **current** bankroll, fractional Kelly (quarter Kelly).
...
**Latest completed epic — FB (Feedback Experience), July 2026 (v1.14).** A 5-issue epic making feedback a visible, first-class channel and adding a short STRUCTURED questionnaire alongside the open form (UM-07) — the owner is actively collecting tester feedback to steer direction, and wanted both spontaneous input AND aggregatable directional signal. Builds on UM-07. **(01)** config-driven questionnaire + NORMALIZED storage: the questions live in `config/settings.yaml` (`feedback.questions` — each `{key, prompt, type ∈ scale/select/multiselect/text, options?}`), so they can be reworded/reordered/added with no code or schema change; answers land in new `feedback_survey` + `feedback_answer` tables (one row per answer value, so a multi-select stores several) keyed by `question_key`, aggregating via a `GROUP BY`; new `src/delivery/views/_feedback_ops.py` (`load_feedback_questions` / `submit_survey` [skips blanks + `None`, rejects all-blank] / `survey_count` / `load_survey_aggregates` / `load_text_answers`); `delete_user` extended to clear a user's surveys+answers (child-first, atomic). **(02)** a dedicated `src/delivery/views/feedback.py` page — **💬 Send feedback** (the open form) + **📋 Quick questions** (renders the config questionnaire dynamically by type) — registered in the nav for all users; the Settings form is replaced by a button→`st.switch_page` (fires only on click, so it's safe outside the nav context, unlike `st.page_link`). **(03)** visibility: the nav gives the always-visible **sidebar** entry, and a fixed-position **floating "💬" button** (positioned via the stable `st-key-` class) on every page opens an `@st.dialog` quick-feedback modal (Streamlit 1.41 has `st.dialog`), opened only on a fresh click so a click-away can't reopen it. **(04)** owner analytics: the Admin Feedback section leads with AGGREGATED questionnaire results — a Plotly bar per scale/select/multiselect question (config-option order; 0..max + a mean for scales) and the raw lines for text questions — above the open messages; owner-gated, every tester-controlled field `html.escape`d. **(05)** a holistic cross-issue review (**APPROVED**) + a Neon round-trip that submitted a questionnaire on the real Postgres backend, aggregated it (multi-select counted per option), and cleaned up (0 rows residue). The open-form owner-email-notify is a single shared `notify_owner_of_feedback` helper. **Shadow-safe: `value_finder.py` ×2 and `predictor.py` byte-for-byte unchanged — feedback only, nowhere near the model/value/prediction path.** New `feedback_survey` / `feedback_answer` tables applied to LOCAL + Neon. Full detail in `betvector_buildplan.md` §FB; outcome ledger in `CLAUDE.md`.

**Prior completed epic — UM (User Management Hardening), July 2026 (v1.13).** An 8-issue (7 features + close) epic giving the owner full lifecycle control over tester accounts from the **Admin** page — motivated by onboarding real testers — plus the visibility/access/feedback utilities a test round needs, and a fix for a real delete bug the WC bet-tracker introduced. This is general account management (not a World Cup epic). **(01)** admin password reset — `auth.admin_reset_password` generates a temp password (shown once via `st.code`, never logged) and *re-arms* `must_change_password=1` (unlike `set_user_password`, which clears it), so a locked-out tester is force-routed to set their own; self-reset blocked. **(02)** edit name & email — `_user_ops.update_user_profile` (name non-empty; email valid, lowercased, unique vs other users — email is the login). **(03)** the **bug fix**: `delete_user` / `clear_bet_history` / `reset_everything` deleted only league `bet_log`, but `wc_bet_log` / `wc_accumulator` / `wc_acca_leg` also carry a NOT-NULL `user_id` FK — so deleting a tester who used the WC tracker threw a FK violation on Neon / orphaned rows on SQLite; a shared `_delete_wc_personal_bets` now sweeps them child-first, atomically (tested with `PRAGMA foreign_keys=ON`). **(04)** role change (`set_user_role`, viewer↔owner) with a **last-owner-demotion guard** (never zero owners → no admin lock-out). **(05)** last-login visibility — new nullable `users.last_login_at` stamped by `auth.record_login` on all three login paths; the Admin table shows last-seen and flags **"never logged in"** (has a password, never signed in). **(06)** force logout / "sign out everywhere" — new `users.session_epoch`; the signed session cookie now embeds the epoch (`uid.epoch.expiry.sig`, backward-compatible with legacy 3-part tokens), and bumping the epoch makes every existing cookie for that user fail the rehydrate epoch-check (the HMAC binds the epoch, so it can't be forged or downgraded — independently security-reviewed). **(07)** feedback capture — new `user_feedback` table; testers submit from Settings → 💬 Send Feedback, the owner reads it in Admin (every field `html.escape`d) with a best-effort owner email-notify. **(08)** an end-to-end lifecycle integration test + a holistic cross-issue review (**APPROVED**, with the auth/token change scrutinised as security-critical). **Owner-gated throughout** (Admin role gate + in-function guards), every write atomic, temp passwords shown once / never logged. **Shadow-safe: `value_finder.py` ×2 and `predictor.py` byte-for-byte unchanged — the epic is entirely account management, nowhere near the model/value/prediction path.** New `users.last_login_at`, `users.session_epoch`, and the `user_feedback` table are idempotent + applied to LOCAL + Neon; existing rows/sessions unaffected. **Deferred to a follow-up epic (owner-agreed): login-as/impersonate + per-user feature flags / a middle role.** Full detail in `betvector_buildplan.md` §UM; outcome ledger in `CLAUDE.md`.

**Prior completed epic — WC-ODDS (Bet-Tracker Odds Auto-Fill + Bookmaker Selection), July 2026 (v1.12).** A 3-issue World Cup epic that stops the owner typing odds by hand in the personal bet-tracker: when a match + market + selection is chosen in the log form or the WC-ACC accumulator slip builder, the odds field **pre-fills from bookmaker prices**, with a **bookmaker selector** — **FanDuel** by default (config `scraping.the_odds_api.default_bookmaker`), any book, or **"Best price"** (highest across books). The field stays editable, so the logged price is always the one actually taken. **(01)** new read-only `src/world_cup/tracker_odds.py` maps the tracker's markets onto the stored `wc_odds` feed — 1X2 → `h2h` by normalized team name / "Draw", Over/Under → `totals` at the 2.5 line — via `wc_odds_lookup` / `wc_odds_books`, picking a specific book or the best price. **(02)** BTTS ("both teams to score") isn't part of the once-daily bulk board pull, so it's fetched **live on demand** — `fetch_btts_odds` does one FREE `/events` lookup then ONE paid `/events/{id}/odds?markets=btts&regions=us` (markets × regions = 1 × 1 = **~1 credit**), parsed by `_parse_btts` and picked by `pick_btts`; the result is **cached per match** (`@st.cache_data`, config TTL `dashboard.cache_ttl_odds_seconds`) so re-picks/reruns are free, and **budget-guarded** (the free `/events` remaining-credit header is checked and the paid call is skipped once ≤ the hard-stop) so on-demand fetching can never blow the monthly quota; a stood-down fetch falls back to clean manual entry and never blocks logging. **(03)** holistic review + live verification + docs. **Shadow-safe / read-only: `value_finder.py` ×2 and `predictor.py` held byte-for-byte unchanged; the stored lookups only READ `wc_odds`, and the live BTTS price is for DISPLAY ONLY and is NEVER written back to `wc_odds`** (writing it there would feed the value finder and start generating BTTS value picks) — the only thing persisted is the user's chosen odds/bookmaker onto the user-scoped `wc_bet_log` / `wc_acca_leg` rows. Live-verified: stored 1X2/O-U auto-fill from Neon, and one real BTTS fetch (Belgium v Senegal) cost exactly **1 credit** (488 → 487) and returned **FanDuel BTTS Yes 1.83 / No 1.95**. No new tables/columns — pure read + on-demand fetch. Full detail in `worldcup_buildplan.md` §WC-ODDS; outcome ledger in `CLAUDE.md`.

**Prior completed epic — WC-QUAL (To Qualify / To Advance), July 2026 (v1.11).** A 4-issue World Cup epic adding a new **"To qualify"** market to the personal bet-tracker (singles + WC-ACC accumulators): a bet on WHICH TEAM ADVANCES from a knockout tie — settled on the winner after 90 min + extra time + penalties — as the counterpart to "Match result (90 min)" (1X2, which settles on the 90-minute score). A knockout won on penalties settles "X to qualify" as a WIN while the same team's 1X2 (90-min) bet is a loss. **(01)** data + settlement engine: new nullable `WCMatch.home_pens`/`away_pens` (the penalty-shootout score, captured by the WC-ACC-02 regulation reconciler from ESPN's `shootoutScore`), a pure `_did_qualify` (advancer = the a.e.t. score tie-broken by the shootout, deferring to *pending* rather than mis-settling when the shootout isn't yet captured), and a market-aware `bet_result(match, market_type, selection)` that routes QUALIFY → advancement and every other market → the existing 90-minute `settlement_score` path (byte-for-behaviour unchanged) — singles AND accumulator legs both flow through it; QUALIFY is knockout-only (rejected on group matches). **(02)** UI: the market dropdown offers "To qualify" only on knockout ties, and 1X2 is relabelled "Match result (90 min)" there (`market_label_for`). **(03)** a **display-only** informational qualify-chance (`P(advance) ≈ P(win 90) + ½·P(draw)` from the stored 90-minute probs — a labelled approximation, never an edge/value signal). **(04)** a holistic money/settlement review + docs. **Manual-odds market — the model does not price it** (only the display-only estimate reads predictions, read-only). **Shadow-safe: `value_finder.py` ×2 and `predictor.py` held byte-for-byte unchanged; the epic writes only the two `wc_matches` pens columns (via the reconciler) + the user-scoped `wc_bet_log`/`wc_acca_leg` tables — nothing touches `wc_predictions`/`wc_value_bets`/the model.** Full detail in `worldcup_buildplan.md` §WC-QUAL; outcome ledger in `CLAUDE.md`.

**Earlier completed epic — WC-ACC (Accumulators / Parlays), July 2026 (v1.10).** A 5-issue World Cup epic extending the WC-BET personal-bet tracker to ACCUMULATORS — 2+ legs as one bet, every leg must win, combined odds = the product of the legs, a void leg drops out of the multiplier, one losing leg loses the bet. A **calculator + tracker, never a recommender**. **(01)** data model (new `wc_accumulator` + `wc_acca_leg` tables) + settlement engine (`accumulator_odds`/`_status`/`_pnl`/`log_wc_accumulator`/`settle_wc_accumulators`/`load_wc_accumulators` in `src/world_cup/bets.py`, reusing `betting.tracker._did_bet_win` per leg); **(02)** knockout **90-minute** settlement — 1X2/O-U/BTTS settle on the regulation score, not extra time/penalties: new `src/world_cup/regulation.py` reconstructs the 90-min score from ESPN `keyEvents` (period-1/2 goals) with a per-team self-check against the official final that **defers rather than mis-settles**, backed by new nullable `WCMatch.home_goals_reg`/`away_goals_reg` + `went_to_extra_time`, and a shared `settlement_score` that **also corrects the WC-BET singles**; **(03)** a bet-slip builder in the 🎟️ My Bets tab with an **informative** combined model-prob + edge readout (independence-assumed, shown only when every leg has a model estimate) and a **same-match correlation warning** (multiplying correlated odds is invalid); **(04)** display of accumulators (a parent row with expandable legs) with singles + accumulators **merged into one running-P&L scoreboard + cumulative chart**, and `settle_wc_accumulators` wired into the morning + evening pipeline after the regulation reconcile; **(05)** a holistic money/settlement review + docs. **Shadow-safe: `value_finder.py` ×2 and `predictor.py` held byte-for-byte unchanged across all 5 issues; the epic only reads model value picks and writes to the user-scoped `wc_accumulator`/`wc_acca_leg`/`wc_bet_log` tables — nothing touches `wc_predictions`/`wc_value_bets`/the model.** New WC-module tables/columns live in the build plan (not §6), per the isolated-`wc_`-module convention. Full detail in `worldcup_buildplan.md` §WC-ACC; outcome ledger in `CLAUDE.md`.

**Prior WC-module epic — WC-BET (Personal Bet Tracker), June 2026 (v1.9).** A 5-issue epic giving each user their own World Cup bet log, entirely separate from the model's shadow value picks: a new user-scoped `wc_bet_log` table + `src/world_cup/bets.py` (`log_wc_bet`/`settle_wc_bets`/`load_wc_bets` with read-time settlement, `wc_bet_summary`, `wc_pnl_timeline`), a 🎟️ My Bets tab (scoreboard, log form, settled-bet list, cumulative-P&L chart), an inline "log this pick" under the value bets (maps a `WCValueBet` → the canonical market, tags it 🎯, freezes model_prob/edge at log time), and pipeline settlement. Bets settle by the same proven `betting.tracker._did_bet_win` logic as league bets. **Shadow-safe and user-scoped; `value_finder.py`/`predictor.py` unchanged.** Full detail in `worldcup_buildplan.md` §WC-BET; outcome ledger in `CLAUDE.md`.

**Prior completed epic — DH (Data Health), June 2026 (v1.8).** A 4-issue operational-monitoring epic answering the question Model Health never does — *is the data actually landing where it should, and fresh?* Motivated by the owner noticing league/WC standings that weren't filling: the classic cause is a rescheduled fixture left as an orphan `scheduled` stub that never flips to `finished`, so the finished-only standings query silently drops it (`cleanup_stale_stubs` sweeps them, but nothing *alarmed* when they appeared). New streamlit-free `src/monitoring/health_check.py` — a **read-only** `run_health_checks()` (every check is a SELECT) returning a `HealthReport` across five groups: **Connectivity** (DB reachable; the Neon-vs-SQLite split-brain tripwire; ingestion API keys present), **Source freshness** (per source, newest-row age vs cadence — odds/results/predictions/stats/Elo/weather/Transfermarkt — plus the Odds API budget; **season-gated** so off-season league staleness SKIPs rather than crying wolf), **Coverage** (upcoming-fixture odds/prediction coverage + finished-but-NULL-goals), **Standings integrity** (past-dated `scheduled` stubs for leagues + WC — the tripwire), and **Pipeline** (the last morning `pipeline_runs` row). Thresholds in `config/settings.yaml` `health:`; the Odds API bounds are reused from `scraping.the_odds_api`. Surfaced three ways (owner choice): **(02)** a CLI `make health` (grouped report, `--json`, `--strict`, exits non-zero on FAIL); **(03)** a 🩺 Data Health dashboard page (green/amber/red cards from the same engine, escaped, AST-tested); **(04)** a guarded morning-pipeline hook that emails the owner (`run_and_alert`, default at WARN) — double-guarded and run post-finalise so it can never break a run. **Entirely read-only (SELECT-only; the sole side effect is the alert email), config-driven, additive, $0; zero change to the model/value/prediction/bet path (`value_finder.py` ×2 and `predictor.py` byte-for-byte unchanged).** On first live run vs Neon it immediately surfaced 2 league + 4 World-Cup stale stubs (the owner's standings concern, confirmed) while correctly staying quiet about the off-season leagues. 69 tests across the four issues. Full detail in `betvector_buildplan.md` §DH; outcome ledger in `CLAUDE.md`.

**Prior completed epic — HC (Help Center), June 2026 (v1.7).** A 6-issue, dashboard-wide in-app user manual plus a downloadable copy, authored once in a new streamlit-free single-source module (`src/delivery/help_content.py`) and rendered by a thin escaped view (`src/delivery/views/help.py`, registered as ❓ Help after Settings). It consolidates the betting knowledge the app had previously assumed: **(01)** a searchable 69-term master glossary (the five scattered page glossaries reconciled, drift resolved) plus a Start-here orientation; **(02)** a screen-by-screen tour — one card per page: what it's for, what to look at first, every badge/colour decoded; **(03)** an FAQ + a per-page "❓ Page guide" deep-link wired centrally in `dashboard.py` (`render_help_link`); **(04)** Betting 101 — ten plain-English lessons, each with a worked numeric example; **(05)** interactive tools (a value calculator, a staking demo, and a scoreline-matrix reader) whose maths is unit-tested against the real value-finder/bankroll formulas and reads the same config bounds; **(06)** a downloadable manual (`build_manual_markdown` + a print-to-PDF `build_manual_html`, no new dependency) and the migration of the five page glossaries (picks / performance / bankroll / match_detail / wc_deep_dive) to read their definitions from the shared source via `glossary_sections_html`, so every term is written exactly once and can never drift. **Entirely read-only / content-driven: zero change to the model, value, prediction or bet-logic path (`value_finder.py` ×2 and `predictor.py` byte-for-byte unchanged), no new tables/columns/data sources, $0 cost.** The Gate-2 faithfulness pass repeatedly caught and corrected real inaccuracies that were latent in the app's own copy (edge is flagged on the raw 1/odds price, not the de-vigged one; the drawdown safety alert fires at 25%, not 30%; flat staking then sized off the *current* bankroll [reversed 2026-06-25: the **code** was corrected so flat sizes off the *starting* bankroll — true level staking, distinct from percentage; see test_flat_level_staking.py]), and the HC-06 migration propagates those fixes to the page glossaries. Full detail in `betvector_buildplan.md` §HC; outcome ledger in `CLAUDE.md`.

**Prior WC-module epic — WC-11A (Player Insight), June 2026 (v1.6).** A 4-issue epic adding a display-only *player* layer to the WC deep dive, all built on a new committed player-rate cache (`src/world_cup/player_rates.py` → `data/world_cup/player_rates.csv.gz`, ~29.8k players derived from the local Transfermarkt datasets: recency-weighted goals-per-90, yellows-per-90, a penalty-taker flag, market value, and an international goals-per-cap fallback; the name resolver blanks on ambiguity rather than guessing). On top of it: **(01)** the rate engine + resolver, with `WCLineup` gaining additive nullable `full_name` + `espn_athlete_id`; **(02)** `research.build_lineup_impact` — a display-only adjusted-xG what-if that rescales the stored Poisson λ by how the confirmed XI's goal-share compares to the team's previous XI (±50% clamp), deep-dive §6; **(03)** `research.build_scorer_board` — each starter's anytime-scorer chance P = 1 − e^(−λ) from his goal-share, ranked, the penalty taker flagged-not-bumped (his spot-kicks already sit in his goals-per-90), §7; **(04)** `research.build_player_watch` — booking-risk (recent club yellows-per-90), star-absence (a high-value/high-scoring player who started the previous XI but not this one), and caps/goals milestones, §8. **Entirely read-only / shadow: the value/staking path (`value_finder.py` ×2) and `predictor.py` were held byte-for-byte unchanged across all 4 issues, no odds are pulled anywhere (zero Odds API credits), and nothing is written back to a model/value table.** One additive WC-table column pair and one committed data artifact; no new external data source and no model change. Full detail in `worldcup_buildplan.md` §WC-11A; outcome ledger in `CLAUDE.md`.

**Prior WC-module epic — DF (Decision-First UX), June 2026 (v1.5).** A 10-issue epic that reorganised the dashboard around the *decision* rather than the data: verdict-led fixtures (WC + leagues), a digestible research card with model-vs-market market blocks, and a read-only WC match deep dive (`src/delivery/views/wc_deep_dive.py`) covering the scoreline matrix, model-vs-every-book line shopping, line movement + CLV, confirmed lineups + rotation flag, group/qualification impact, and a per-match Bayesian-vs-Poisson read with a glossary. **Entirely read-only / shadow: no new tables, columns, data sources, or model changes** — the value/staking path (`value_finder.py`) and `predictor.py` were held byte-for-byte unchanged across all 10 issues, and the Bayesian read is display-only (promotion stays manual, §11). Full detail in `worldcup_buildplan.md` §DF-01..DF-10; outcome ledger in `CLAUDE.md`.
