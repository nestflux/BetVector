...
st.markdown(
    '<p class="text-muted">Configure your staking, thresholds, and notifications</p>',
    unsafe_allow_html=True,
)
st.divider()
...
if not user_data:
    ...
    # ==================================================================
    # Section: Send Feedback — link to the dedicated Feedback page (FB-02)
    # ==================================================================
    # The open form + questionnaire now live on their own visible Feedback page; this
    # is just a pointer so the Settings entry still leads there (no duplicate form).
    st.divider()
    st.markdown(
    ...
    st.markdown(
        f'<p style="font-family: Inter, sans-serif; font-size: 13px; '
        f'color: {COLOURS["text_secondary"]}; margin-bottom: 12px;">'
        f'Found a bug or have an idea? Head to the Feedback page — an open form plus a '
        f'few quick questions.'
        f'</p>',
        unsafe_allow_html=True,
    )
    # A button + switch_page (not st.page_link): switch_page fires only on click, so
    # it never throws when settings.py is rendered outside the nav context (tests),
    # matching the dashboard's existing render_help_link pattern.
    if st.button("💬 Open the Feedback page", key="settings_go_feedback"):
